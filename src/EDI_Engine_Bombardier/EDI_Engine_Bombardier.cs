...
namespace EDI
{
  ...
  public sealed class EDI_Engine_Bombardier : EDI_Engine_Base
  {
      ...
      while (m_DeliveryFileReader.Read())
      {
        ...
        else if (fileline[0] == "310")
        {
          ...
          Transaction trans = new Transaction {
            ...
            buyer = buyer,
            po = po,
            salesOrder = this.GetSalesOrderForPartAndPoAndLine(
              partNumber, po, sCustomer, line),
      ...
      if (fileTransactionCount == transactions.Count.ToString())
      {
        ...
        foreach (Transaction trans in transactions)
        {
            ...
            DoesCustomerHaveAddressInJbDb(trans.customer, trans.shipTo))
          {
            ...
            if(!trans.salesOrder.Equals("Nouveau Sales_Order"))
            {
      ...
      ref string address)
    {
      ...
      insertSoQuery = insertSoQuery.Replace("@customerPo", po);
      insertSoQuery = insertSoQuery.Replace("@customer", trans.customer);

      insertSoQuery = insertSoQuery.Replace("@shipTo", address);

      insertSoQuery = insertSoQuery.Replace("@terms", terms);

      var contact = GetContactForCustomerAndContactName(trans.customer,
        trans.buyer);
      bool contactIsNumeric = int.TryParse(contact, out int contactId);
      if(contactIsNumeric)
      {
        insertSoQuery = insertSoQuery.Replace("@contact", contact);
      }
      else
      {
      ...
      {
        insertSoQuery = insertSoQuery.Replace("@orderDate",
          DateTime.Now.ToString());
      }
      else
      {
        insertSoQuery = insertSoQuery.Replace("@orderDate", orderDate.Substring(0, 4)
          + "-" + orderDate.Substring(4, 2) + "-" + orderDate.Substring(6));
      }

      if (String.IsNullOrWhiteSpace(promisedDate))
      {
        insertSoQuery = insertSoQuery.Replace("@promisedDate",
          DateTime.Now.ToString());
      }
      else
      {
        insertSoQuery = insertSoQuery.Replace("@promisedDate", promisedDate.Substring(0, 4)
          + "-" + promisedDate.Substring(4, 2) + "-" + promisedDate.Substring(6));
      }

      insertSoQuery = insertSoQuery.Replace("@status", M_SO_CREATION_STATUS);
      insertSoQuery = insertSoQuery.Replace("@noteText",
        trans.freeFormMessage.Replace("'", "''"));
      ...
      ref string promisedDate, ref string address)
    {
      ...
      insertSoDetailQuery = insertSoDetailQuery.Replace("@line", trans.line);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@partNumber", trans.partNumber);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@shipTo", address);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@status", M_SO_CREATION_STATUS);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@unitPrice", trans.unitPrice);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@totalPrice",
       (Convert.ToDouble(trans.qty) *
        Convert.ToDouble(trans.unitPrice)).ToString());
      insertSoDetailQuery = insertSoDetailQuery.Replace(
        "@orderQty", trans.qty);
      insertSoDetailQuery = insertSoDetailQuery.Replace(
        "@defferedQty", trans.qty);
      insertSoDetailQuery = insertSoDetailQuery.Replace(
      ...
      {
        insertSoDetailQuery = insertSoDetailQuery.Replace(
          "@promisedDate", DateTime.Now.ToString());
      }
      else
      {
        insertSoDetailQuery = insertSoDetailQuery.Replace(
          "@promisedDate", promisedDate.Substring(0, 4)
          + "-" + promisedDate.Substring(4, 2) + "-"
          + promisedDate.Substring(6));
      }

      insertSoDetailQuery = insertSoDetailQuery.Replace(
        "@extDesc", trans.extendedDescription);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@noteText",
        "Rev: " + trans.rev + "\nDessin: " + trans.dr);
      insertSoDetailQuery = insertSoDetailQuery.Replace("@description",
        trans.description.Length>=30?trans.description.Substring(0, 30):trans.description);
      ...
      ref string terms, ref string promisedDate, ref string address)
    {
      ...
      updateSoQuery = updateSoQuery.Replace("@salesOrder", trans.salesOrder);
      updateSoQuery = updateSoQuery.Replace("@shipTo", address);
      updateSoQuery = updateSoQuery.Replace("@terms", terms);
      if (String.IsNullOrWhiteSpace(promisedDate))
      {
        updateSoQuery = updateSoQuery.Replace("@promisedDate",
          DateTime.Now.ToString());
      }
      else
      {
        updateSoQuery = updateSoQuery.Replace("@promisedDate",
          promisedDate.Substring(0, 4) + "-" +
          promisedDate.Substring(4, 2) + "-" + promisedDate.Substring(6));
      }
      updateSoQuery = updateSoQuery.Replace("@status", M_SO_CREATION_STATUS);
      if(!String.IsNullOrWhiteSpace(trans.freeFormMessage))
      {
        updateSoQuery = updateSoQuery.Replace("@noteText",
          "CONCAT(Note_Text, CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10) + '"
          + trans.freeFormMessage.Replace("'", "''") + "'");
      }
      else
      {
        updateSoQuery = updateSoQuery.Replace("@noteText", "Note_Text");
      ...
      ref string soDetail, ref string promisedDate, ref string address)
    {
      ...
      + "Order_Qty=@orderQty, Last_Updated='@lastUpdated', "
      + "Promised_Date='@promisedDate', Note_Text='@noteText', "
      + "Ext_Description='@extDesc', Description='@description' "
      + "WHERE SO_Detail = '@soDetail'";
      updateSoDetailQuery = updateSoDetailQuery.Replace("@line", trans.line);
      updateSoDetailQuery = updateSoDetailQuery.Replace("@shipTo", address);

      updateSoDetailQuery = updateSoDetailQuery.Replace("@status", M_SO_CREATION_STATUS);

      updateSoDetailQuery = updateSoDetailQuery.Replace("@unitPrice", trans.unitPrice);
      updateSoDetailQuery = updateSoDetailQuery.Replace("@totalPrice",
       (Convert.ToDouble(trans.qty) *
        Convert.ToDouble(trans.unitPrice)).ToString());
      updateSoDetailQuery = updateSoDetailQuery.Replace("@orderQty",
       trans.qty);
      updateSoDetailQuery = updateSoDetailQuery.Replace("@lastUpdated"
        , DateTime.Now.ToString());

            // TODO @mond: What do we do when we have no promised date????????
      if (String.IsNullOrWhiteSpace(promisedDate))
      {
        updateSoDetailQuery = updateSoDetailQuery.Replace("@promisedDate",
          DateTime.Now.ToString());
      }
      else
      {
        updateSoDetailQuery = updateSoDetailQuery.Replace(
          "@promisedDate", promisedDate.Substring(0, 4)
          + "-" + promisedDate.Substring(4, 2) + "-"
          + promisedDate.Substring(6));
      }
      ...
      updateSoDetailQuery = updateSoDetailQuery.Replace("@extDesc",
        trans.extendedDescription);

      updateSoDetailQuery = updateSoDetailQuery.Replace("@description",
        trans.description.Length>=30?trans.description.Substring(0, 30):trans.description);

      updateSoDetailQuery = updateSoDetailQuery.Replace(
        "@noteText", "Rev: " + trans.rev + "\nDessin: " + trans.dr);
    ...
    public override void ExecuteExtraFeature(string featureName, string[] args = null)
    {
      ...
      else if (featureName == "850Exec") { Exec850(args); }
      else if (featureName == "855") { Exec855(args); }
      else if (featureName == "860") { Analyze860(args); }
      else if (featureName == "860Exec") { Exec860(args); }
    ...
    public void Exec850(string[] args)
    {
      ...
      foreach (DataRow row in extraFeaturesTable.Rows)
      {
        ...
        //InsertOrUpdateDeliveryForSoDetail(string sod, string date, string qty, string shippingInstructions = "");
        ...
        command.Parameters.AddWithValue("@createdSO", soNumber.ToString());
        command.Parameters.AddWithValue("@releaseNumber"
          , row["ReleaseNumber"].ToString());
      ...
      try
      {
        if (readerForEightFiftySalesOrders.HasRows)
        {
            ...
            in tableEightFiftySalesOrders.Rows)
          {
            ...
            + "Status FROM SO_Header WHERE Sales_Order LIKE '@soNumber'";

            var jbSalesOrderDetailQuery = "SELECT TOP 1 "
            + "Material AS PartNumber, SO_Line "
            + "FROM SO_Detail WHERE Sales_Order LIKE '@soNumber'";

            jbSalesOrderHeaderQuery = jbSalesOrderHeaderQuery.Replace(
              "@soNumber", currentRowFromEightFifty["SO"].ToString());
            jbSalesOrderDetailQuery = jbSalesOrderDetailQuery.Replace(
              "@soNumber", currentRowFromEightFifty["SO"].ToString());

            var dtJbSoInfo = conn.GetData(jbSalesOrderHeaderQuery);
            if (dtJbSoInfo.Rows.Count < 1) { return; }

            var dtJbSoDetailInfo = conn.GetData(jbSalesOrderDetailQuery);
            if (dtJbSoDetailInfo.Rows.Count < 1) { return; }
            ...
            if (!headerHasBeenCreated)
            {
              ...
              line100 = line100.Replace("@PoNumber",
                rowJbSoInfo["Customer_PO"].ToString());
              line100 = line100.Replace("@date", todayDate);
              line100 = line100.Replace("@850releaseNumber",
                currentRowFromEightFifty["releaseNumber"].ToString());
          ...
          String.Format("UPDATE brpCreatedSalesOrders SET "
            + "tempStatus = '855' WHERE id in ({0})", eightFiftyIds);
    ...
    public void Exec860(string[] args)
    {
      ...
      foreach (DataRow row in extraFeaturesTable.Rows)
      {
        ...
        if(row["Sales_Order"].Equals("Nouveau Sales_Order"))
        {
          ...
          updateSoQuery = updateSoQuery.Replace("@so", row["Sales_Order"].ToString());
          updateSoDetailQuery = updateSoDetailQuery.Replace("@so", row["Sales_Order"].ToString());
        }
        ...
        InsertOrUpdateDeliveryForSoDetail(row["SO_Detail"].ToString(),
          ...
          !PoIs550(row["Customer_PO"].ToString())?"PPAP":"", insert);
        ...
        command.Parameters.AddWithValue("@modifiedSO", row["Sales_Order"]);
        command.Parameters.AddWithValue("@releaseNumber"
          , row["ReleaseNumber"].ToString());
        command.ExecuteNonQuery();
      ...
      try
      {
        if (readerForEightSixtySalesOrders.HasRows)
        {
            ...
            in tableEightSixtySalesOrders.Rows)
          {
            ...
            jbSalesOrderHeaderQuery = jbSalesOrderHeaderQuery.Replace(
              "@soNumber", currentRow["SO"].ToString());
            jbSalesOrderDetailQuery = jbSalesOrderDetailQuery.Replace(
              "@soNumber", currentRow["SO"].ToString());
            ...
            if (!headerHasBeenCreated)
            {
              ...
              line100 = line100.Replace("@PoNumber",
                rowJbSoInfo["Customer_PO"].ToString());
              line100 = line100.Replace("@date", todayDate);
              line100 = line100.Replace("@860releaseNumber",
                currentRow["releaseNumber"].ToString());
          ...
          String.Format("UPDATE brpModifiedSalesOrders SET "
            + "tempStatus = '865' WHERE id in ({0})", eightSixtyIds);
      ...
      ref DataRow[] poRows, ref DataTable mDataTable, ref int currentRowIndex)
    {
      for (var i = 0; i < poRows.Length; i++)
      {
        ...
        newRow[3] = "0";
        newRow[8] = mDataTable.Rows[currentRowIndex][9];

        mDataTable.Rows.Add(newRow);
        PerformCalculationsForRow(ref mDataTable, currentRowIndex + i);
      }
    }

    private void InsertOrUpdateDeliveryForSoDetail(string sod, string date,
      string qty, string shippingInstructions = "", bool insert = false)
    {
      var conn = new jbConnection(this.M_DBNAME, this.M_DBSERVER);
      ...
      if(insert)
      {
        sQuery = "INSERT INTO Delivery (SO_Detail, Requested_Date, Promised_Date, "
        ...
        sQuery += ") VALUES (" + sod + ", '" + date + "', '" + date + "', " + qty
        + " , " + qty + ", '" + Guid.NewGuid() + "'";

        if(!String.IsNullOrWhiteSpace(shippingInstructions))
        {
          sQuery += ", '" + shippingInstructions + "'";
        }
      ...
      {
        sQuery = "UPDATE Delivery SET Requested_Date='" + date + "', Promised_Date='" + date
        + "', Promised_Quantity=" + qty + ", Remaining_Quantity=" + qty;

        if(!String.IsNullOrWhiteSpace(shippingInstructions))
        {
          sQuery += ", Comment='" + shippingInstructions + "'";
        }

        sQuery += " WHERE " + "SO_Detail = " + sod;
      }
    ...
    private void FetchCustomersForFiles(ref Dictionary<string, string> customersDict)
    {
      ...
      foreach (DataRow row in _dtFilesAndInfos.Rows)
      {
        ...
        var sFullQuery = sPartialQuery + " Customer LIKE '" + sCustomer
        + "' AND Address.Ship_To_ID = '" + row["plant"] + "'";
    ...
    {
      var sFullQuery = sPartialQuery + " Address.Ship_To_ID = '" + sPlant + "'";
      var conn = new jbConnection(this.M_DBNAME, this.M_DBSERVER);
    ...
    private string GetSingleSoDetailForSalesOrder(in string so)
    {
      ...
      + "SO_Detail.Sales_Order LIKE '@so'";

      query = query.Replace("@so", so);
    ...
    new void AddSODetailLine(DataRow dr, string sSONumber, string sLine)
    {
      ...
      var address = conn.GetData("SELECT Address FROM Address WHERE Customer LIKE '"
       + M_CUSTOMER_ID + "' AND Ship_To_ID LIKE '" + dr[13] + "'");
      ...
      var bHasStringWithSingleQuote = false || sMaterial.Contains("'");

      string sQuery;

      if (bHasStringWithSingleQuote)
      {
        var sFixedMat = sMaterial.Replace("'", "''");
      ...
      else
      {
        ...
        sQuery += ") VALUES ('" + sSONumber + "', '" + sLine + "', " + addressID + ", '" + sStatus
        + "', 'M', " + sUnitPrice + ", '" + sPriceUofM + "', " + sTotalPrice + ", "
        + sOrderQty + ", '" + sStockUofM + "'" + ", " + sOrderQty + ", '" + sPromisedDate
        + "', '" + sMaterial + "', '" + sCostUofM + "', '" + Guid.NewGuid() + "', 0, 0, 0";
      ...
      + "Remaining_Quantity, ObjectID) VALUES (" + dtLatestSoDetail.Rows[0][0] + ", '"
      + sPromisedDate + "', '" + sPromisedDate + "', " + sOrderQty + " , " + sOrderQty
      + ", '" + Guid.NewGuid() + "')";
    ...
    private int MergeJbWithCurrentPartAndPo(ref DataTable tempTable, ref DataTable mDataTable, ref int currentRowIndex)
    {
      ...
      var sTempSelect = "Customer_PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "'";

      if (sClient != "" || sClient != null)
      {
        M_CUSTOMER_ID = sClient;
        sTempSelect += " AND Client = '" + M_CUSTOMER_ID + "'";
      }
      ...
      if (dtJbSo == null) //No sales order in JB, fill placeholder data to set creation for write operation
      {
      ...
      else
      {
        ...
        else if (dtJbSo.Rows.Count == 1)
        {
          ...
          + "WHERE SO_Detail.Sales_Order = '" + sJbSo + "' AND (SO_Detail.Material LIKE '" + sCurrentPart + "') "
    ...
    {
      var sQuery = "SELECT SO_Detail FROM Delivery WHERE SO_Detail = " + row[16];
      ...
      + "LEFT JOIN SO_Header ON SO_Detail.Sales_Order = SO_Header.Sales_Order "
      + "LEFT JOIN Customer_Part ON SO_Header.Customer = Customer_Part.Customer AND SO_Detail.Material = Customer_Part.Material "
      + "WHERE SO_Detail.SO_Detail = " + row[16];
      ...
      sSelect = "SELECT Sales_Code FROM Material WHERE Material LIKE '" + row[0] + "'";
    ...
    {
      var conn = new jbConnection(this.M_DBNAME, this.M_DBSERVER);
      var sQuery = "SELECT SO_Detail FROM Delivery WHERE SO_Detail = " + jbRow[16];
      ...
      + "LEFT JOIN SO_Header ON SO_Detail.Sales_Order = SO_Header.Sales_Order "
      + "LEFT JOIN Customer_Part ON SO_Header.Customer = Customer_Part.Customer AND SO_Detail.Material = Customer_Part.Material "
      + "WHERE SO_Detail.SO_Detail = " + jbRow[16];
      ...
      sSelect = "SELECT Sales_Code FROM Material WHERE Material LIKE '" + jbRow[0] + "'";
      var dTSalesCode = conn.GetData(sSelect);
      var oSalesCode = dTSalesCode.Rows[0][0];
      ...
      string newPrice, object salesCode, string soDetail, bool bDeliveryUpdate)
    {
      ...
      sUpdate += "Promised_Date = '" + date + "'";
      sUpdateDelivery += "Promised_Date = '" + date + "' , Requested_Date = '" + date + "' ";

      sUpdate += ", Order_Qty = " + qty + ", Deferred_Qty = " + qty;
      sUpdateDelivery += ", Promised_Quantity = " + qty + ", Remaining_Quantity = " + qty + " ";

      sUpdate += ", Unit_Price = " + unitPrice;
      sUpdate += ", Total_Price = " + newPrice;
      sUpdate += ", SO_Line = '" + line + "'";

      if (salesCode != DBNull.Value)
      {
        sUpdate += ", Sales_Code = '" + salesCode + "' ";
      }

      sUpdate += ", Status = '" + M_SO_CREATION_STATUS + "'";

      sUpdate += " WHERE SO_Detail = " + soDetail;
      sUpdateDelivery += "WHERE SO_Detail = " + soDetail;
      ...
      if (!bDeliveryUpdate)
      {
        ...
        sUpdateDelivery += " (" + soDetail + ", '" + date + "' , '" + date + "' ," + qty
        + ", " + qty + ", '" + Guid.NewGuid() + "')";
      }
    ...
    public override void Write()
    {
      ...
      while (iCurrentLine < m_MergedTable.Rows.Count)
      {
        ...
        var sTempSelect = "Customer_PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "'";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var poRows = m_MergedTable.Select(sTempSelect);

        sTempSelect = "Customer_PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "' AND Source = 'JB' AND SO_Detail_Status <> 'Shipped'";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var jbNotShipped = m_MergedTable.Select(sTempSelect);

        sTempSelect = "Customer_PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "' AND Source = 'CSV'";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var csvOnlyRows = m_MergedTable.Select(sTempSelect);

        sTempSelect = "Customer_PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "' AND Source NOT IN ('JB', 'CSV')";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        ...
        if (sSo != "")
        {
          ...
          if (jbNotShipped.Length != csvOnlyRows.Length)
          {
            ...
            else
            {
              ...
              {
                conn.SetData("DELETE FROM Delivery WHERE Delivery.SO_Detail = " + jbNotShipped[x][16]);
                conn.SetData("DELETE FROM SO_Detail WHERE SO_Detail.SO_Detail = " + jbNotShipped[x][16]);
              }
            }
          }
        }
    ...
    public override void Write(object obj)
    {
      ...
      foreach (DataGridViewRow dgvrow in datagrid.Rows)
      {
      ...
      while (gridTable.Rows.Count > 0)
      {
        ...
        var sTempSelect = "PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "'";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var poRows = gridTable.Select(sTempSelect);

        sTempSelect = "PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "' AND Source = 'JB' AND [SO Detail Status] <> 'Shipped'";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var jbNotShipped = gridTable.Select(sTempSelect);

        sTempSelect = "PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "' AND Source = 'CSV'";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var csvOnlyRows = gridTable.Select(sTempSelect);

        sTempSelect = "PO = '" + sCurrentPo + "' AND Material = '" + sCurrentPart + "' AND Source NOT IN ('JB', 'CSV')";
        if (sClient != "") { sTempSelect += " AND Client = '" + sClient + "'"; }
        var jbAndCsvRows = gridTable.Select(sTempSelect);

        if (sSo != "")
        {
          for (var x = 0; x < jbAndCsvRows.Length; x++)
          {
            if (jbAndCsvRows[x]["SO Detail Status"].ToString() != "Shipped")
            {
              UpdateRow(ref jbAndCsvRows[x]);
            }
          }
